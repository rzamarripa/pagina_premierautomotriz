...
$(function () {
  ...
  function validateForm() {
    ...
    if (name.length === 0) {
      showFieldError("#name", validationMessages.name.required);
      isValid = false;
    } else if (name.length < 3) {
      showFieldError("#name", validationMessages.name.minlength);
      isValid = false;
    } else {
    ...
    if (email.length === 0) {
      showFieldError("#email", validationMessages.email.required);
      isValid = false;
    } else if (!emailRegex.test(email)) {
      showFieldError("#email", validationMessages.email.email);
      isValid = false;
    } else {
    ...
    if (phone.length === 0) {
      showFieldError("#phone", validationMessages.phone.required);
      isValid = false;
    } else if (!phoneRegex.test(phone.replace(/[\s\-()]/g, ""))) {
      showFieldError("#phone", validationMessages.phone.minlength);
      isValid = false;
    } else {
    ...
    if (message.length === 0) {
      showFieldError("#message", validationMessages.message.required);
      isValid = false;
    } else if (message.length < 10) {
      showFieldError("#message", validationMessages.message.minlength);
      isValid = false;
    } else {
  ...
  function showLoading() {
    const loadingHTML = `
            <div class="alert alert-info" style="display: flex; align-items: center; gap: 15px;">
                <i class="fa fa-spinner fa-spin fa-2x"></i>
                <div>
                    <strong>Enviando mensaje...</strong><br>
                    <small>Por favor espera un momento</small>
                </div>
            </div>
        `;
    $("#success").html(loadingHTML);
  ...
  function showSuccessMessage(name) {
    ...
    const successHTML = `
            <div class="alert alert-success" style="animation: fadeIn 0.5s;">
                ...
                <i class="fa fa-check-circle" style="font-size: 2em; color: #28a745; margin-bottom: 10px;"></i>
                <h4><strong>¡Mensaje Enviado Exitosamente!</strong></h4>
                <p>Gracias <strong>${firstName}</strong>, hemos recibido tu mensaje.</p>
  ...
  function showErrorMessage(error) {
    const errorHTML = `
            <div class="alert alert-danger" style="animation: fadeIn 0.5s;">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                <i class="fa fa-exclamation-triangle" style="font-size: 2em; color: #dc3545; margin-bottom: 10px;"></i>
                <h4><strong>Error al Enviar el Mensaje</strong></h4>
                <p>${
                  error ||
                  "Lo sentimos, ocurrió un error al procesar tu solicitud. Por favor intenta nuevamente."
                }</p>
                <p>Si el problema persiste, contáctanos directamente al <a href="tel:+526678464561">667 846 45 61</a></p>
            </div>
        `;
  ...
  $("#contactForm").on("submit", function (e) {
    ...
    const formData = {
      ...
      service: "Contacto desde sitio web",
    };
